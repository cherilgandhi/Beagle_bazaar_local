...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not is_user_exit:
                try:
                    methods.register(form,phone_num)
...
def admin_profiles(request):
    ...
    users = []
    # print(form_map.FORM_MAP)
    for account_type in form_map.FORM_MAP:
        print(account_type)
        print(form_map.MASKS[account_type])
...
def pincode_api(request):
    pin_code = str(request.GET.get('pin_code', None))
    country = str(request.GET.get('country', None))
    # url = 'http://www.postalpincode.in/api/pincode/' + pin_code
    url = 'https://api.worldpostallocations.com?postalcode='+pin_code+'&countrycode='+country
    response = requests.get(url)
    if response.status_code == 200:
        data = response.text
    else:
...
def user_info(request):
    ...
    for mata in matadata :
        ...
        try:
            ...
            dict ={
                'user' : model.user.username,
                'unit_number': model.unit_number if  model.unit_number != '' else '-',
                'street_address':model.street_address if  model.street_address != '' else '-',
                'tel_number':model.tel_number if  model.tel_number != '' else '-',
                'country':model.country if  model.country != '' else '-',
                'pin_code':model.pin_code if  model.pin_code != '' else '-',
                'city':model.city if  model.city != '' else '-',
                'state':model.state if  model.state != '' else '-',
                'effective_year':model.effective_year if  model.effective_year != '' else '-',
                'effective_month':model.effective_month if  model.effective_month != '' else '-'
            }
            final_dict = dict
            single_dealer_data_dict[dict_name] = final_dict
        except:
...
@login_required
def distributor_info(request):
    address_data = []
    # get user to get user data
    matadata = models.metadata.Metadata.objects.filter(account_type = 5)
    dealer_email_list =[]
    final_list = []
    for mata in matadata :
        user_final_dict ={}
        ...
        try:
            ...
            single_dealer_data_dict[dict_name] = final_dict
            print('!@##@$@$%$#%',single_dealer_data_dict[dict_name])
        except:
            pass
