...
def level_status(request, modules, meta):
    status = []
    for level in range(1, 4):
        # print('outer forloop level ->',level)
        count = 0
        verified_count = 0
        counter = 0
        for i in range(len(modules)):
            # print('array [',level,'][',i,'] => ','level = ',modules[i].level,'| is_completed(step) ->',meta.is_complete(i),'| array[',modules[i].level,'] == ',level,'->',modules[i].level == level)
            '''
                given if condition check the level is not completed by user. 
                then it increment the 
                count by +1
                
                and
                counter is equal to count
            '''
            # print('array [',level,'][',i,'] => ')
            ...
            if not meta.is_complete(i) and modules[i].level == level:
                ...
                count += 1
                counter = level
                # print('incomplete level->',level,'count',count)
                # print('level = counter',counter)
                
                '''
                    the given elif condition check the if the user has completed the level and 
                    the profile verification by admin is pending then it increment the varified_count 
                    by +1 
                '''
            elif not meta.is_verified(i) and modules[i].level == level:
                # print('enter elif')
                # print('array [',level,'][',i,'] => ','level = ',modules[i].level,'| is_verified(step) ->',meta.is_verified(i),'| array[',modules[i].level,'] == ',level,'->',modules[i].level == level)
                verified_count += 1
        status.append({
            'level': level,
            'count': count,
