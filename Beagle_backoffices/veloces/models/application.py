...
class VeloceApplication(models.Model):
    user = models.ForeignKey(AuthUser, related_name="application_by", on_delete=models.CASCADE)
    ...
    rejected_by = models.OneToOneField(AuthUser, on_delete=models.CASCADE, null=True, blank=True)
...
class ReviewedVeloceApplication(models.Model):
    ...
    reviewed_by = models.ForeignKey(AuthUser, related_name="approved_by", null=True, blank=True, on_delete=models.SET_NULL)
    is_reviewed = models.BooleanField(default=False)
    is_approved = models.BooleanField(default=False)
    is_rejected = models.BooleanField(default=False)
    rejected_by = models.OneToOneField(AuthUser, on_delete=models.CASCADE, null=True, blank=True)
...
class ApplicationStep1(models.Model):
    ...
    coborrower = models.ForeignKey(AuthUser, models.CASCADE, related_name="coborrower", blank=True, null=True)
...
class InvoiceStep1(models.Model):
    application = models.OneToOneField(VeloceApplication, on_delete=models.CASCADE)
    customer = models.ForeignKey(AuthUser, models.CASCADE)
...
class DealerSpecialVeloceMarkup(models.Model):
    ...
    markup_percentage = models.DecimalField(decimal_places=2, max_digits=6, null=True, blank=True)
    markup_payment_by = models.CharField(choices=MARKUP_PAYMENT_BY, default="1", max_length=1)
    dealer = models.OneToOneField(AuthUser, related_name="dealer", on_delete=models.CASCADE)
