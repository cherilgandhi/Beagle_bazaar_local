...
def home(request):
    # que = bizcred_user.AuthUser.objects.using('bizcred_db').filter(email = request.user)
    # que = models.auth_user.AuthUser.objects.using('bizcred_db').filter(email = request.user)
    # print('$$$$$$$$$$$$$$$$$$$$$$$$$',que)    
    return HttpResponse('this is bizcred first page')
