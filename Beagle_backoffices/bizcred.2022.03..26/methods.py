...
def level_approved_email(user, level, account_type, email_info, request):
    print('user level approve',user)
    admin_user = request.user
    user_info = bizcred_models.auth_user.AuthUser.objects.using('bizcred_db').get(username=user)
    ...
    if acc_type == 'LENDER':
        if level == 2:
            ...
            if user_info:
                ...
                message = get_template('bizcred/mail/level-approved-user.html').render(data)
                ...
                message = get_template('bizcred/mail/level-approved-admin.html').render(ctx)
...
def updated_info_approved_email(title, meta, request):
    ...
    if acc_type == 'LENDER':
        ...
        message = get_template('bizcred/mail/form-updation-admin.html').render(ctx)
...
def rejected_info_email(user, meta, title, module):
    ...
    # user_info = User.objects.get(username=request.user)
    acc_type = enums.AccountType(meta.account_type).name
    step_model = module.model.objects.using('bizcred_db').get(user=user)
    ...
    if acc_type == 'LENDER':
        ...
        message = get_template('bizcred/mail/form-rejection-user-email.html').render(ctx)
