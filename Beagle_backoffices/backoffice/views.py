...
from . models import BackofficePermission
User = get_user_model()
...
import backoffice.methods as backoffice_methods                        #|
...
from veloce import enums as store_enums                                #|
...
def query(request):
    ...
    # user_permissions=""
    # try:
    #     user_permissions = backoffice_modules.permissions.BackofficePermission.objects.using('auth_db').get(user = user)
    # except:
    #     user_permissions =''
    ...
    # else:
    #     print('query not get')
    #     backoffice_modules.permissions.BackofficePermission.objects.using('auth_db').create(user= request.user)

    return render(request,'backoffice/tril-query.html')
            ...
            if not is_user_exit:
                try:
                    ...
                    backoffice_modules.credentials.UserCredentials.objects.using('auth_db').create(user = User.objects.get(email=email),
                      ...
                      # permission
                    user_per = User.objects.get(email=email)
                    user_permissions=""
                    try:
                        user_permissions = backoffice_modules.permissions.BackofficePermission.objects.using('auth_db').get(user = user_per)
                    except:
                        user_permissions =''

                    if user_permissions:
                        print('default permission to be declear here',user_permissions)
                    else:
                        print('create')
                        backoffice_modules.permissions.BackofficePermission.objects.using('auth_db').create(user= user_per)
                    # permission end
...
def check_email_exist(request):
    if request.method == 'POST':  
        ...
        if filtered_email.count() > 0:
            return JsonResponse(True,safe=False)
        else:
            return JsonResponse('Email Address Not exist',safe=False)
...
def filter_user_on_portal_by_account_type(account_type):
    ...
    for email in user_email:
        ...
        address_modules = bizcred_models.address.Address.objects.using('bizcred_db').filter(user = request_user)
        if address_modules.count() == 1:
            address_module = address_modules[0]
            country_enums = bizcred_enums.countries
            for country_enum in country_enums:
                if str(address_module.country) == country_enum[0]:
                    address_country = country_enum[1]
            address = {
                ...
                'state': address_module.state,
                'country':address_country.capitalize()
            }
        else:
            address = {
                'unit_number':'',
...
def user_permission_check(request,permission_name):
    list = [permission_name,]
    # print(list)
    user = User.objects.get(pk = request.user.pk)
    permission_data = backoffice_modules.permissions.BackofficePermission.objects.using('auth_db').filter(user = user,**{list[0]: True})
    # print('def permission data ', permission_data)
    if permission_data :
        return True
    else:
        return False
...
def user_customer(request):
    # customer profile view permission
    permission_status = user_permission_check(request,'customer_profile_view')
    if permission_status == False: return redirect('backoffice:index')
...
def user_company(request):
    ...
    permission_status = user_permission_check(request,'company_profile_view')
    if permission_status == False: return redirect('backoffice:index')
    ...
    #  many = BackofficePermission.objects.all()
...
def user_distributor(request):
    # distributors profile view permission
    permission_status = user_permission_check(request,'distributors_profile_view')
    if permission_status == False: return redirect('backoffice:index')
...
def user_dealer(request):
    # dealer profile view permission
    permission_status = user_permission_check(request,'dealer_profile_view')
    if permission_status == False: return redirect('backoffice:index')
...
def user_service_agent(request):
    # serviceagent profile view permission
    permission_status = user_permission_check(request,'serviceagent_profile_view')
    if permission_status == False: return redirect('backoffice:index')
...
def user_lender(request):
    # lender profile view permission
    permission_status = user_permission_check(request,'lender_profile_view')
    if permission_status == False: return redirect('backoffice:index')
...
def user_pending_profile(request,account_string):
    ...
    # customer profile approve view permission
    if user_t == 1:
        permission_status = user_permission_check(request,'customer_profile_approve_view')
        if permission_status == False: return redirect('backoffice:index')
    
    # company profile approve view permission
    if user_t == 2:
        permission_status = user_permission_check(request,'company_profile_approve_view')
        if permission_status == False: return redirect('backoffice:index')
        
    # lender profile approve view permission
    if user_t == 3:
        permission_status = user_permission_check(request,'lender_profile_approve_view')
        if permission_status == False: return redirect('backoffice:index')
        
    # dealer profile approve view permission
    if user_t == 4:
        permission_status = user_permission_check(request,'dealer_profile_approve_view')
        if permission_status == False: return redirect('backoffice:index')
    
    # distributors profile approve view permission
    if user_t == 5:
        permission_status = user_permission_check(request,'distributors_profile_approve_view')
        if permission_status == False: return redirect('backoffice:index')
    
    # serviceagent profile approve view permission
    if user_t == 6:
        permission_status = user_permission_check(request,'serviceagent_profile_approve_view')
        if permission_status == False: return redirect('backoffice:index')
...
def product_list(request):
    ...
    permission_status = user_permission_check(request,'product_view')
    if permission_status == False: return redirect('backoffice:index')
    ...
    products_counts = all_product.count()
    if filter_category:
...
def view_profile(request,pk):
    ...
    permission_data =backoffice_modules.permissions.BackofficePermission.objects.using('auth_db').get(user = single_user)
    permission_values = model_to_dict(permission_data)
    ...
    for key, value in permission_values.items():
        ...
        main_dict[key] = single_dict

    context ={
        'user_profiles': user,
        'permission_dict':main_dict,
        'single_users':single_user,
        
    }
    return render(request,'backoffice/admin-templates/view-profile.html',context)

def ajax_user_permission(request):
    if request.method =='POST':
        status = ""
        alart_class=""
        ...
        two_list = [field_name,ck_value]
                   
        user = User.objects.get(pk = user_id)
        permission_data = backoffice_modules.permissions.BackofficePermission.objects.using('auth_db').filter(user = user).update(**{two_list[0]: ck_value})
        if permission_data:
            status += '{} field is {}'.format(str(field_name),str(ck_value))
            alart_class = 'success'
            return JsonResponse({'messages':status,'alart_class':alart_class})
        else:
            status += "field is not updated please contact developer"
            alart_class = 'danger'
            return JsonResponse({'messages':status,'alart_class':alart_class})
...
def category_create(request):
    ...
    permission_status = user_permission_check(request,'category_create')
    if permission_status == False: return redirect('backoffice:index')
...
def category_edit(request,slug):
    # category delete permission
    permission_status = user_permission_check(request,'category_update')
    if permission_status == False: return redirect('backoffice:index')
...
def category_list(request):
    ...
    permission_status = user_permission_check(request,'category_view')
    if permission_status == False: return redirect('backoffice:index')
...
def category_delete(request,slug):
    # category delete permission
    permission_status = user_permission_check(request,'category_delete')
    if permission_status == False: return redirect('backoffice:index')
    ...
    if subcategory.count() == 0:
        ...
        messages.error(request,'Category has SubCategory Please delete It. <a href="/subcategory-list/">click me</a> to delete the Subcategory !!')
    return redirect('backoffice:category_list')
...
def subcategory_create(request):
    # subcategory create permission
    permission_status = user_permission_check(request,'subcategory_create')
    if permission_status == False: return redirect('backoffice:index')
...
def subcategory_edit(request,slug):
    # subcategory update permission
    permission_status = user_permission_check(request,'subcategory_update')
    if permission_status == False: return redirect('backoffice:index')
...
def subcategory_list(request):
    # subcategory view permission
    permission_status = user_permission_check(request,'subcategory_view')
    if permission_status == False: return redirect('backoffice:index')
...
def subcategory_delete(request,slug):
    # subcategory delete permission
    permission_status = user_permission_check(request,'subcategory_delete')
    if permission_status == False: return redirect('backoffice:index')
    ...
    if product.count() == 0:
        ...
        messages.error(request,'Your Product is listed with SubCategory Please delete It. <a href="/product-list/">click me</a> to delete the Product !!')
    return redirect('backoffice:subcategory_list')
...
def create_product(request):
    ...
    permission_status = user_permission_check(request,'product_create')
    if permission_status == False: return redirect('backoffice:index')
...
def edit_product(request,pk):
    ...
    permission_status = user_permission_check(request,'product_update')
    if permission_status == False: return redirect('backoffice:index')
...
def make_image_as_feature_img(request, pk):
    try:
        get_images_data = store_models.ProductMedia.objects.using('veloce1_db').get(pk=pk)
...
def all_orders(request):
    ...
    permission_status = user_permission_check(request,'order_view')
    if permission_status == False: return redirect('backoffice:index')
    
    unique_email_lists = []
    orders_id_lists = []
    order_status_enums = enums.to_choices(store_enums.OrderStatus)
    
    order_status_filter = request.GET.get('order_status')
    order_account_email_filter = request.GET.get('account_email')
    order_id_filter = request.GET.get('order_id')
    order_date_filter = request.GET.get('order_date')
    print(order_date_filter)
    
    orders = store_models.listings.UserOrder.objects.using('veloce1_db').all().order_by('-id')
    orders_counts = orders.count()
    
    if order_status_filter:
        if order_status_filter != 'all':
            orders = orders.filter(Q(order_status = int(order_status_filter)))
    
    if order_account_email_filter:
        if order_account_email_filter != 'all':
            orders = orders.filter(Q(user__email= str(order_account_email_filter)))

    if order_id_filter:   
        if order_id_filter != 'all':
            orders = orders.filter(Q(order_id= str(order_id_filter)))
            print('no')
    
    if order_date_filter:
        if len(order_date_filter) >= 10:
            order_date_formatting = order_date_filter.split('/')
            compare_order_date = datetime.datetime.strptime(str(order_date_formatting[0])+str(order_date_formatting[1])+str(order_date_formatting[2]), '%d%m%Y').date()
            orders = orders.filter(Q(created_at__date = compare_order_date))
        
    
    order_lists =[]
    for order in orders:
        # making order unique email lists
        if order.user.email not in unique_email_lists:
            unique_email_lists.append(order.user.email)
            
        # making order id lists
        orders_id_lists.append(order.order_id)  
        
        total_products = store_models.listings.UserOrderProductList.objects.using('veloce1_db').filter(user_order = order)
        ...
        single_order_dict ={
            ...
            'payment_status':str(store_enums.PaymentStatus(order.payment_status).name).replace('_',' '),
            'payment_method':order.payment_method,
            'transaction_id':order.transaction_id,
            'order_status':str(store_enums.OrderStatus(order.order_status).name).replace('_',' '),
            'total_transaction_amount':order.total_transaction_amount,
            'total_products_count':total_products_count,
            'created_at':order.created_at,
        } 
        # print(single_order_dict)
        order_lists.append(single_order_dict)
        
    customer_emails = unique_email_lists
    
    
    page_num = request.GET.get('page', 1)
    paginator = Paginator(order_lists, 10)
    try:
        all_orders = paginator.page(page_num)
    except PageNotAnInteger:
    ...
    context ={  
        ...
        'orders_id_lists':orders_id_lists,
        
        # recents filters variables
        'order_status_filter':order_status_filter,
        'order_account_email_filter':order_account_email_filter,
        'order_id_filter':order_id_filter,
        'order_date_filter':order_date_filter
    }
    
    return render(request,'backoffice/user-templates/product-all-orders.html',context)
    
def ajax_email_by_order_status(request):
    order_status = request.POST.get('order_status')
    orders = store_models.listings.UserOrder.objects.using('veloce1_db').all().order_by('-id')
    unique_email_lists=[]
    if order_status:
        if order_status != 'all':
            orders = orders.filter(Q(order_status = int(order_status)))
        else:
            orders = orders
        html= '<option selected value="all">All Email</option>'
        for order in orders:
            if order.user.email not in unique_email_lists:
                unique_email_lists.append(order.user.email)
                html += '<option value="{0}">{1}</option>'.format(order.user.email,order.user.email)
    return JsonResponse({'data':html})

def ajax_order_id_by_email(request):
    order_status = request.POST.get('order_status')
    email_filter = request.POST.get('email_filter')
    if order_status != 'all':
        orders = store_models.listings.UserOrder.objects.using('veloce1_db').filter(order_status = order_status).order_by('-id')
    else:
        orders = store_models.listings.UserOrder.objects.using('veloce1_db').all().order_by('-id')
        
    if email_filter:
        if email_filter != 'all':
            orders = orders.filter(Q(user__email = email_filter))
        else:
            orders = orders
        html= '<option selected value="all">All Orders</option>'
        
        for order in orders:
            view_order_id = '# '+str(order.order_id)
            html += '<option value="{0}">{1}</option>'.format(order.order_id,view_order_id)
    return JsonResponse({'data':html})

def ajax_order_date_by_order_id(request):
    status = 'multiple'
    order_status = request.POST.get('order_status')
    email_filter = request.POST.get('email_filter')
    order_id_filter = request.POST.get('order_id_filter')
    ...
    if order_id_filter:
        if order_id_filter != 'all':
            # send the date of the selected date
            status = 'single'
            orders = orders.filter(Q(order_id = order_id_filter))
        else:
            # let the user to add the date
            status = 'multiple'
            orders = orders
        ...
        if status == 'single': 
            for order in orders:
                order_date = date_format(order.created_at,'d/m/Y')
                data = str(order_date)
        else:
            data = "Add the date if you want to filter records by date."
...
def orders_view(request,pk):
    ...
    permission_status = user_permission_check(request,'order_view')
    if permission_status == False: return redirect('backoffice:index')
    order_details = store_models.listings.UserOrder.objects.using('veloce1_db').get(pk = pk)
    order_products = store_models.listings.UserOrderProductList.objects.using('veloce1_db').filter(user_order = pk)
    order_address = store_models.listings.UserOrderAddress.objects.using('veloce1_db').get(user_order = pk)
    ...
    return render(request,'backoffice/user-templates/product-order-view.html',context)
...
def user_profile(request,pk):
    ...
    # company
    # customer
    request_user = bizcred_models.auth_user.AuthUser.objects.using('bizcred_db').get(pk = pk)
    user_address = bizcred_models.address.Address.objects.using('bizcred_db').get(user = request_user)
    print(request_user,user_address)
    return render(request,'backoffice/user-templates/user-profile.html',{'request_user':request_user,'user_address':user_address})